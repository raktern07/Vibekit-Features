...
(function(window, document) {
  ...
  if (window.VibikitChatbot) {
    console.warn('Maxxit Chatbot already initialized');
    return;
  }
  ...
  window.VibikitChatbot = {
    ...
    init: function(config = {}) {
      ...
      console.log('Maxxit Chatbot initialized');
    },
    ...
    createChatWindow: function(config) {
      ...
      header.innerHTML = `
        ...
        <button id="maxxit-close-btn" style="background: none; border: none; color: white; cursor: pointer; font-size: 20px;">&times;</button>
      `;
      ...
      header.querySelector('#maxxit-close-btn').addEventListener('click', () => {
        this.closeChat();
      });
    ...
    loadStyles: function() {
      if (document.getElementById('maxxit-chatbot-styles')) return;
      
      const style = document.createElement('style');
      style.id = 'maxxit-chatbot-styles';
      style.textContent = `
        @keyframes pulse {
          0%, 100% { opacity: 1; }
          50% { opacity: 0.5; }
